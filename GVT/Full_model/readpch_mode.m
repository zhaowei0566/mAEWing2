function ModeShape=readpch_mode(pchfname,minodeid,maxnodeid,Modelimit)
...
%             eval(['modeshape' num2str(mode) '(1,1:7)=[NaturalFrequency(' num2str(mode) '),0,0,0,0,0,0];']);
            ...
            while currentFPI<EOF && flag && nodeid<=maxnodeid
                ...
                if isempty(nodeid)==1 
                    ...
                    ModeShape.modeshape(jj,1,mode)=str2num(linef06(1:10));
                    
%                     str2num(linef06(20:72))
                    
                    ModeShape.modeshape(jj,2:4,mode)=str2num(linef06(20:72));
%                     eval(['modeshape' num2str(mode) '(' num2str(jj) ',1:4)=[nodeid,str2num(linef06(20:72))];']);
                    
                    linef06=fgetl(fid111);currentFPI=ftell(fid111);
                    
%                     str2num(linef06(20:72))
                    ModeShape.modeshape(jj,5:7,mode)=str2num(linef06(20:72));
...
%             eval(['ModeShape.modeshape' num2str(mode) '=modeshape' num2str(mode) ';']);
        else
            modeflag=0;
        end
    end
